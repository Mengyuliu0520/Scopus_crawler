...
manu_vec = tfidf_vectorizer.fit_transform(manu['original_abstruct_text'])
print("size:  ")
print(tfidf.shape)
print("done in %0.3fs." % (time() - t0))
...
nmf1 = NMF(n_components=n_components, random_state=1,
          beta_loss='kullback-leibler', solver='mu', max_iter=1000, alpha=.1,
          l1_ratio=.5).fit(manu_vec)
print("\nTopics in NMF model (generalized Kullback-Leibler divergence):")
tfidf_feature_names = tfidf_vectorizer.get_feature_names()
print_top_words(nmf, tfidf_feature_names, n_top_words)
review_topic = nmf.fit_transform(tfidf)
manu_topic = nmf1.fit_transform(manu_vec)
res = cdi(review_topic,manu_topic)
print(res.shape)
