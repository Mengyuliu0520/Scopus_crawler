...
res = cdi(mt,rt)
print(res.shape)
reviewer_id = list(doc['reviewer'])
pos = []
pre = reviewer_id[0]
for i in range(len(reviewer_id)):
    if pre==reviewer_id[i]:
        pre = reviewer_id[i]
    else:
        pos.append(i)
        pre = reviewer_id[i]
average_ds = np.zeros((res.shape[0], len(pos)+1))
current = 0
for i in range(res.shape[0]):
    for j in range(len(pos)):
        average_ds[i,j] = np.average(res[i,current:pos[j]])
        current = pos[j]
    average_ds[i,-1] = np.average(res[i,current:])
    current = 0
print(average_ds)
rank = np.argsort(average_ds)
